...
//    @GradedTest(name = "test05_CreateTable_Sync()", max_score = 10)
    ...
    protected void verifyOrderConsistent(String table, int key) {
        ...
        for (String node : servers) {
        	System.out.println("printing node in grader "+node);
            ResultSet result = session.execute(readResultFromTableCmd(key, DEFAULT_TABLE_NAME, node));
            System.out.println("printing result in grader "+result);
            
            results[i] = "";
            for (Row row : result) {
                results[i] += row;
                System.out.println("Printingg the result checking "+row);
                nonEmpty = true;
            }
            i++;
        }
        ...
        for (String result : results) {
            if (!results[0].equals(result)) {
            	System.out.println("printing results "+results[0]+result);
                match = false;
